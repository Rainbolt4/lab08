...
{
	if (subtree == NULL) { return 0; }  // tree is empty return 0
	else return 1 +  size(subtree->left_) + size(subtree->right_); // tree is not empty return 1 + size of each of the nodes
}
   ...
   BinaryTree:: height( const BinaryNode * subtree )
{
	...
	else if ((subtree->right_ != NULL) && (subtree->left_ != NULL)) // there is a left and right subtree we want to get the height of the tallest tree only
	{
		if (height(subtree->left_) > height(subtree->right_))
		{
			return 1 + height(subtree->left_); // left subtree is the tallest add 1 and return the height of that tree
		}
		else
		{
			return 1 + height(subtree->right_); // right subtree is the tallest add 1 and return the height of that tree
		}


	}
	else if ((subtree->right_ != NULL) && (subtree->left_ == NULL)) // there is a right tree but no left tree
	{
		return 1 + height(subtree->right_);
	}
	else if ((subtree->right_ == NULL) && (subtree->left_ != NULL)) // there is a left tree but no left tree
	{
   ...
   BinaryTree:: leftmost( const BinaryNode * subtree )
{
	...
	return subtree->entry_;                 // this returns the left most value as far as left vs right pointers are concerned but when looking at the 
}                                           // dispaly printed from the display function the node this prints appears to be the right most, I chould change the left_ to right_
                                            // but I feel this is what the question is asking for so this is what I have provided. 
